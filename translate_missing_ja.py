...
print("Loading missing_ja.json...")
with open('locales/missing_ja.json', encoding='utf-8') as f:
    missing_data = json.load(f)

print(f"Found {len(missing_data)} missing keys")

print("Loading ja.json...")
with open('locales/ja.json', encoding='utf-8') as f:
    ja_data = json.load(f)
...
for key, value in missing_data.items():
    ...
    translated_value = translate_value(value, 'ja')
    set_nested(ja_data, key, translated_value)

print(f"Completed: {count}/{len(missing_data)}")

print("Saving ja.json...")
