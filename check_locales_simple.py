...
def main():
    ...
    for filename in sorted(language_files):
        ...
        try:
            ...
            if not missing_keys:
                results.append((lang_code, 0, []))
                print(f"✓ {lang_code}: All keys present")
            else:
                results.append((lang_code, len(missing_keys), list(missing_keys)[:10]))
    ...
    print("=" * 50)
    total_missing = 0
    for lang_code, missing_count, sample_missing in results:
        if missing_count > 0:
            total_missing += missing_count
            print(f"{lang_code}: {missing_count} missing keys")
        else:
            print(f"{lang_code}: OK")
    
    print(f"\nTotal missing keys across all languages: {total_missing}")
